...
- V7 trainable-quantum case-study status now has three artifact rows:
  - older documented V7: `67.35%` best validation and `65.02%` test
  - April 6, 2026 resumed Colab L4 rerun: `72.89%` best validation and `72.53%` test, reconstructed from a user-captured Colab log with local checkpoint sync
  - April 27-28, 2026 clean non-resumed Colab L4 rerun: `69.97%` best validation and `65.88%` test, reconstructed from captured `colab_v7_rerun_clean.ipynb` output because the runtime disconnected before the artifact-copy cell ran
...
- The main unresolved V7 item is artifact provenance cleanup for missing copied JSON/`experiments/v7_*` metadata, not a default new training run. The April 27 Drive folder contains checkpoints, but its `experiments/` subfolder is empty.
...
- `train_v7.py` threads `--target` through and now emits aggregate-compatible future V7 JSON rows; the existing April 6 and April 27 V7 rerun JSON rows still note that they were reconstructed from captured Colab output rather than copied directly from Drive.
