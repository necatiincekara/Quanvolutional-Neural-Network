...
def run_record(experiment_id, kind, manifest, protocol, source_paths, *, manifest_path=None):
    env=environment()
    manifest_path=Path(manifest_path or ROOT/'research/e0_e3_20260915/dataset_manifest.json').resolve()
    manifest_reference=str(manifest_path.relative_to(ROOT)) if manifest_path.is_relative_to(ROOT) else str(manifest_path)
    return {'schema_version':1,'experiment_id':experiment_id,'kind':kind,'status':'running',
            ...
            'data':{'manifest':manifest_reference,
                    'manifest_checksum':manifest['manifest_checksum'],
